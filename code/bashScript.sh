...
do
  ./sim_cache 32 ${n} ${j} 0 0 0 gcc_trace.txt > graph2${i}${j}.txt
    ...
    variable2=$(($i+10));
    variable3=$(($j));
    echo "$variable3,$variable2,$variable" >> graph2.csv
  echo "Looping ... number $n $i $j"
done
done
...
do
  ...
  variable3=$(($i+10));
  variable4=$(($j));
   echo "$variable4,$variable3,$variable2,$variable" >> graph3.csv
  echo "Looping ... number $n $i $j"
done
done
...
do
  ...
  then
    ...
    variable3=$(($i+10));
    variable4=$(($j));
    echo "$variable4,$variable3,$variable2,$variable" >> graph5.csv
    echo "Looping ... number $n $i $j"
  fi
done
done
...
do
  ...
  variable3=$(($i+10));
  variable5=$(($j));
    echo "1,$variable5,$variable3,$variable2,$variable4,$variable" >> graph6.csv
  echo "Looping ... number $n $i $j"
 ...
 variable=$(grep 'h.' graph6${i}2.txt | awk '{print $6}');
  ...
  variable3=$(($i+10));
    echo "2,0,$variable3,$variable2,$variable4,$variable" >> graph6.csv
  echo "Looping ... number $n $i $j"
  ...
  variable3=$(($i+10));
    echo "4,0,$variable3,$variable2,$variable4,$variable" >> graph6.csv
  echo "Looping ... number $n $i $j"
